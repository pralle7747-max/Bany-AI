{
  "version": "1.0.1",
  "url": "https://raw.githubusercontent.com/pralle7747-max/Bany-AI/main/app_new.exe"
}
import os
import sys
import sqlite3
...
def check_and_apply_update():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if os.path.exists(current_file) and current_file.endswith(".py"):
                with open(current_file, "r", encoding="utf-8") as f:
                    local_code = f.read()

                if online_code.strip() != local_code.strip():
                    show_update_dialog(current_file, online_code)
    except Exception as e:
        print(f"Update-Prüfung übersprungen: {e}")

def show_update_dialog(current_file, online_code):
    root = ctk.CTk()
    root.title("Bany AI - Auto Updater")
    ...
    root.geometry(f"+{(sw-450)//2}+{(sh-220)//2}")

    lbl_title = ctk.CTkLabel(root, text="🔄 Neues Update für Bany AI!", font=("Arial", 16, "bold"), text_color="#FFE01B")
    lbl_title.pack(pady=(20, 10))

    lbl_status = ctk.CTkLabel(root, text="Installiere neue Software-Version...", font=("Arial", 12))
    lbl_status.pack(pady=5)

    progress = ctk.CTkProgressBar(root, width=350, progress_color="#FFE01B")
    ...
    def perform_update():
        ...
        with open(current_file, "w", encoding="utf-8") as f:
            f.write(online_code)
        progress.set(1.0)
        lbl_status.configure(text="✅ Update erfolgreich installiert! Bitte App neu starten.")
        btn_restart = ctk.CTkButton(root, text="Beenden & Neu Starten", fg_color="#FFE01B", text_color="black", command=sys.exit)
...
class AetherOSAssistant(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        self.logo_label = ctk.CTkLabel(brand_frame, text="✨ Bany AI", font=("Arial", 18, "bold"), text_color="#FFE01B")
